...
Harness.test({
  ...
  mssql: {
    text  : 'SELECT ([post].[content] + @param1) FROM [post] WHERE ([post].[userId] IN (SELECT [customer].[id] FROM [customer]))',
    string: 'SELECT ([post].[content] + \'!\') FROM [post] WHERE ([post].[userId] IN (SELECT [customer].[id] FROM [customer]))'
  },
  params: ['!']
});
...
Harness.test({
  ...
  mssql: {
    text  : 'SELECT (([post].[id] + @param1) + [post].[content]) FROM [post] WHERE ([post].[userId] NOT IN (SELECT [customer].[id] FROM [customer]))',
    string: 'SELECT (([post].[id] + \': \') + [post].[content]) FROM [post] WHERE ([post].[userId] NOT IN (SELECT [customer].[id] FROM [customer]))'
  },
  params: [': ']
});
