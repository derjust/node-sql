...
Harness.test({
  ...
  mssql: {
    text  : 'SELECT ([customer].[name] + [customer].[age]) AS [nameAndAge] FROM [customer] WHERE (([customer].[age] > @param1) AND ([customer].[age] < @param2))',
    string: 'SELECT ([customer].[name] + [customer].[age]) AS [nameAndAge] FROM [customer] WHERE (([customer].[age] > 10) AND ([customer].[age] < 20))'
  },
  params: [10, 20]
});
...
Harness.test({
  ...
  mssql: {
    text  : 'SELECT ([customer].[age] BETWEEN @param1 AND @param2) AS [ageBetween] FROM [customer]',
    string: 'SELECT ([customer].[age] BETWEEN 10 AND 20) AS [ageBetween] FROM [customer]'
  },
  params: [10, 20]
});
